# Contributing

This documentation is maintained by the TED Open Data team at the [Publications Office of the EU](https://op.europa.eu/). Contributions and corrections from the community are welcome.

## How to contribute

1. Fork the repository or create a branch from `develop`
2. Make your changes
3. Open a pull request targeting the `develop` branch
4. Your changes will be reviewed by the team and published with the next release (when `develop` is merged to `main`)

Please discuss significant changes via a [GitHub issue](https://github.com/OP-TED/ted-rdf-docs/issues) before starting work.

## Contributing to the Query Library

The Query Library provides example SPARQL queries that are displayed directly in the [TED Open Data Service](https://data.ted.europa.eu/). You can help by:

- **Adding comments** to existing queries to make them easier to understand
- **Fixing or improving** existing queries (e.g. adding explicit joins for better performance)
- **Contributing new queries** that demonstrate useful data retrieval patterns

The queries are no longer kept here. They live in [OP-TED/ted-open-data-examples](https://github.com/OP-TED/ted-open-data-examples), which the service reads directly, and contributions go there — see its [contributing guide](https://github.com/OP-TED/ted-open-data-examples/blob/main/CONTRIBUTING.md) for the file format, the writing guidelines and how to submit a query.

## Licence

The documentation and sample code in this repository are licensed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/deed.en).
