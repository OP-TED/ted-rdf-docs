...
|folder contains mapping suite packages organised based on the standard forms numbers. Their name is formed based on the form number (e.g. F03, F06) prefixed with package_ for readability. When the eForms will be mapped, then the corresponding appropriate organisation will be chosen.
folder contains xref:mapping_suite/mapping-suite-structure.adoc [mapping suite packages]
