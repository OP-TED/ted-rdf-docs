...
TED Open Data Web Service (TED-SWS) is a process pipeline system that continuously
...
The TED Open Data Web Service (TED-SWS) connects the TED infrastructure for the collection and publication of public procurement notices with the infrastructure of http://data.europa.eu/[data.europa.eu] 
...
.Mapping Suites
****
The TED-RDF Mappings are the transformation rules needed by the TED-RDF Conversion Pipeline (both of which are part of the TED Open Data Web Services, aka TED-SWS system) to convert TED notices available in XML format to RDF.
