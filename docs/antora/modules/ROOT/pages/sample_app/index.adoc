...
Understanding of RDF and SPARQL:: Familiarity with RDF (Resource Description Framework) and SPARQL (SPARQL Protocol and RDF Query Language) is crucial. TED-SWS provides data in RDF format and utilises SPARQL for querying.
...
Understanding of EU Procurement Data:: If your goal is to work with specific types of EU procurement data, such as contract notices or award notices, it's important to have a basic understanding of these concepts, and the https://docs.ted.europa.eu/EPO/latest/index.html[eProcurement ontology].

The sample applications can be accessed via the *Reusing semantic web service artefacts* menu on the left.
