...
writing notebook documents. To work with R and Jupiter Notebook, it is first necessary to install some tools and applications. After installation, R code can be used to perform a query on the CELLAR database, and display the results in tabular format.

Before proceeding, ensure that Python and Jupyter Notebook are already installed. This can be done using the tutorial in <<SWS:ROOT:sample_app/jupyter_notebook_python.adoc#, this>> section.
