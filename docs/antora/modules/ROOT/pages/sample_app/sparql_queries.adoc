...
A query is submitted to the SPARQL end point by replacing the text highlighted below in blue in the Query text box below with your own query:
...
== Query: How many contract awards (count), published on a given date were awarded to suppliers who are SMEs, for a particular country?
{nbsp}
...
This SPARQL query retrieves information about contract award notices that have been published by buyers from Germany  published on TED on 05-09-2023.
...
== Query: List all winners published on a given date
{nbsp}
...
== Query: List the contract values for contract awards published on a particilar date. Order the results by value descending and filter by highest value.
{nbsp}

This SPARQL query retrieves the procurement notice, its publication date, the corresponding lot, and the highest awarded amount along with the currency code for a specific date ("20230905" or 05-09-2023). It does so by specifying a value for the date and then querying the RDF data based on that value. The query returns the information in descending order of the awarded amount, limiting the results to one row, with the effect that only the highest value is displayed.
