...
This document shows an examplein Python using Jupyter Notebook.
Jupyter Notebook is an application for creating and sharing
computational documents. Python is a programming language for
writing computational documents. To use Jupyter Notebook and Python, it is first
necessary to install some tools and applications to run the Python code that will
perform the query to the CELLAR database, and display the results in tabular
format.
