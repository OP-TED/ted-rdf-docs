...
[#_write_rml_rules]
==== Write RML rules
If in the previous step we have defined the mapping rules in YARRRML, then this step consists of the simple action of executing xref:mapping_suite/toolchain.adoc[the tool that generates RML out of YARRRML].
