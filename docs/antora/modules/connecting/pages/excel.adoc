...
The first part is described on the page: xref:sparql.adoc[Submitting a query in the TED SPARQL Endpoint].
...
. Start by running the query in the TED SPARQL EndPoint. Copy the query below by clicking on the icon on the top right of the code block.
