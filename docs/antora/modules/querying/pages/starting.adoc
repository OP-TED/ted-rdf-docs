...
The EU makes public procurement data in RDF format available for querying. A pipeline process converts XML notice data into RDF format which is stored in a public procurement RDF database, also known as a triple store, called the "Cellar", maintained by the Publications Office.
